import { Leader } from "../interface/leader";

export const leaders: Leader[] = [
    // Giai đoạn 1858-1945: Kháng chiến chống Pháp
    {
        id: "1858-1945",
        name: "Trương Định",
        birth: "1820-1864",
        img: "https://ordi.vn/wp-content/uploads/2024/08/49996816842_d27a10b30c_o.jpg",
        description:
            "Trương Định là thủ lĩnh nghĩa quân Nam Kỳ, kiên quyết chống Pháp sau khi Gia Định thất thủ, giữ vững ý chí kháng chiến.",
        battleYear: "1861-1864",
        battleDescription:
            "Thời kỳ Pháp xâm lược Nam Kỳ, Trương Định lãnh đạo nghĩa quân chống Pháp sau khi Gia Định thất thủ. Ông tổ chức nhiều trận đánh lớn tại Gò Công và Tân Hòa, gây thiệt hại nặng cho địch và trở thành biểu tượng kháng chiến kiên cường."
    },

    {
        id: "1858-1945",
        name: "Phan Đình Phùng",
        birth: "1847-1896",
        img: "https://cdn.nhandan.vn/images/78d92bfef5333421c1cfa9f19aa2572af2f6454a381555b801846adcfda2022145c540fb9bfd40f62245885bd9113c240e9e2d66d48e61b3599239903431eb56ab8798562102821c7b7344f384421ba2/1cc908a58ec10eafe48563baaefd8fb4.jpg",
        description:
            "Phan Đình Phùng là lãnh tụ phong trào Cần Vương, nổi bật với khí tiết thanh liêm và quyết tâm chống Pháp.",
        battleYear: "1885-1896",
        battleDescription:
            "Trong phong trào Cần Vương, ông tổ chức nhiều trận đánh lớn tại Hà Tĩnh và Nghệ An. Nghĩa quân nhiều lần đẩy lui quân Pháp, đặc biệt tại Tân Sở và Hàm Rồng, gây thiệt hại lớn và duy trì cuộc kháng chiến suốt nhiều năm."
    },

    {
        id: "1858-1945",
        name: "Hoàng Hoa Thám (Đề Thám)",
        birth: "1858-1913",
        img: "https://static.wixstatic.com/media/41a9b2_21008072eeb24452815265f188abca75~mv2.png/v1/fill/w_640,h_950,al_c,q_90,usm_0.66_1.00_0.01,enc_avif,quality_auto/41a9b2_21008072eeb24452815265f188abca75~mv2.png",
        description:
            "Đề Thám là thủ lĩnh khởi nghĩa Yên Thế, nổi tiếng với chiến thuật du kích linh hoạt chống Pháp.",
        battleYear: "1887-1913",
        battleDescription:
            "Khởi nghĩa Yên Thế kéo dài hơn 25 năm dưới sự chỉ huy của Đề Thám. Nghĩa quân tiến hành nhiều trận phục kích lớn, gây thiệt hại cho quân Pháp và buộc địch nhiều lần thương lượng nhưng không thể khuất phục tinh thần kháng chiến."
    },

    {
        id: "1858-1945",
        name: "Nguyễn Trung Trực",
        birth: "1838-1868",
        img: "https://www.baolongan.vn/image/news/2015/20150930/fckimage/Nguyen_Trung_Truc_portrait-(1).jpg",
        description:
            "Nguyễn Trung Trực là anh hùng Nam Bộ, nổi tiếng với tinh thần bất khuất và chiến công đánh chìm tàu Pháp.",
        battleYear: "1861-1868",
        battleDescription:
            "Ông chỉ huy trận Nhật Tảo, đốt cháy tàu Espérance gây chấn động lớn. Sau đó tiếp tục chiến đấu tại Rạch Giá và Gò Công, khiến quân Pháp nhiều lần thất bại và được nhân dân tôn kính vì lòng trung nghĩa."
    },

    {
        id: "1858-1945",
        name: "Nguyễn Thái Học",
        birth: "1902-1930",
        img: "https://upload.wikimedia.org/wikipedia/commons/0/02/Nguy%E1%BB%85n_Th%C3%A1i_H%E1%BB%8Dc.jpg",
        description:
            "Nguyễn Thái Học là lãnh tụ Việt Nam Quốc dân Đảng, chủ trương đấu tranh vũ trang giành độc lập.",
        battleYear: "1930",
        battleDescription:
            "Ông lãnh đạo khởi nghĩa Yên Bái nhằm đánh đổ ách thống trị Pháp. Dù thất bại, phong trào gây tiếng vang lớn và khơi dậy tinh thần yêu nước mạnh mẽ. Khí phách của ông khi bước lên pháp trường đã trở thành biểu tượng bất khuất."
    },

    {
        id: "1858-1945",
        name: "Hồ Chí Minh",
        birth: "1890-1969",
        img: "https://cdn.baohatinh.vn/images/93923d58fc3bb0f0af3e4594b0c37b71d566b88ce44aeca5cd1e2e1d5190f17044007254023cb0832b1739c5b9800072/77d5063829t3920l49.jpg",
        description:
            "Hồ Chí Minh là lãnh tụ cách mạng Việt Nam, người dẫn dắt dân tộc giành độc lập và lập nên nước Việt Nam mới.",
        battleYear: "1940-1945",
        battleDescription:
            "Trong thời kỳ kháng Nhật, Người xây dựng lực lượng Việt Minh, phát động phong trào đấu tranh rộng khắp. Sự chuẩn bị chiến lược của Người tạo thời cơ quyết định cho Tổng khởi nghĩa Tháng Tám, mở ra kỷ nguyên độc lập dân tộc."
    },

    // Giai đoạn 1945-1975: Kháng chiến chống Mỹ cứu nước
    {
        id: "1945-1975",
        name: "Võ Nguyên Giáp",
        birth: "1911-2013",
        img: "https://images.hcmcpv.org.vn/res/news/2021/08/20-08-2021-vo-nguyen-giap-vi-nhan-tuong-tan-trung-bao-quoc-9D356295.jpg",
        description:
            "Võ Nguyên Giáp là Đại tướng đầu tiên của Việt Nam, chỉ huy nhiều chiến dịch quyết định trong hai cuộc kháng chiến.",
        battleYear: "1946-1975",
        battleDescription:
            "Ông chỉ huy chiến dịch Điện Biên Phủ vang dội, đánh bại thực dân Pháp năm 1954. Trong kháng chiến chống Mỹ, ông góp phần hoạch định các chiến dịch lớn, dẫn đến Đại thắng Mùa Xuân 1975, giải phóng hoàn toàn miền Nam."
    },

    {
        id: "1945-1975",
        name: "Nguyễn Chí Thanh",
        birth: "1914-1967",
        img: "https://baoquangbinh.vn/dataimages/201312/original/images565395_anh4.jpg",
        description:
            "Nguyễn Chí Thanh là nhà lãnh đạo quân sự tài năng, góp phần xây dựng lực lượng chiến đấu tại miền Nam.",
        battleYear: "1965-1967",
        battleDescription:
            "Trên cương vị chỉ huy chiến trường miền Nam, ông tổ chức các chiến dịch lớn chống Mỹ, củng cố lực lượng du kích và tạo thế tấn công mạnh mẽ. Tư duy quân sự chủ động của ông góp phần làm thay đổi cục diện chiến tranh."
    },

    {
        id: "1945-1975",
        name: "Lê Trọng Tấn",
        birth: "1914-1986",
        img: "https://upload.wikimedia.org/wikipedia/commons/4/48/Dai-tuong-le-trong-tan.jpg",
        description:
            "Lê Trọng Tấn là tướng lĩnh xuất sắc, tham gia chỉ huy nhiều chiến dịch quan trọng trong kháng chiến.",
        battleYear: "1954-1975",
        battleDescription:
            "Ông góp công lớn trong chiến dịch Điện Biên Phủ và sau đó là các chiến dịch giải phóng miền Nam. Năm 1975, ông trực tiếp chỉ huy cánh quân hướng Đông trong chiến dịch Hồ Chí Minh, góp phần giải phóng Sài Gòn."
    },

    {
        id: "1945-1975",
        name: "Trần Văn Trà",
        birth: "1918-1996",
        img: "https://baophuyen.vn/upload/Images/2019/thang09/24/tra.jpg",
        description:
            "Trần Văn Trà là tư lệnh quân Giải phóng miền Nam, nổi bật với khả năng chỉ huy chiến trường.",
        battleYear: "1965-1975",
        battleDescription:
            "Ông giữ vai trò chủ chốt trong nhiều chiến dịch lớn, đặc biệt là chiến dịch Hồ Chí Minh. Với tầm nhìn chiến lược, ông tổ chức phối hợp lực lượng từ các hướng, góp phần quyết định vào thắng lợi giải phóng miền Nam."
    },

    {
        id: "1945-1975",
        name: "Phạm Hùng",
        birth: "1912-1988",
        img: "https://tulieuvankien.dangcongsan.vn/upload/3000006/20251024/11430284920d55e8ccf5ce548cfe50ffPham_Hung.jpg",
        description:
            "Phạm Hùng là lãnh đạo cách mạng miền Nam, kiên định trong đấu tranh chính trị và quân sự.",
        battleYear: "1960-1975",
        battleDescription:
            "Ông điều hành các phong trào đấu tranh ở miền Nam, kết hợp lực lượng chính trị và quân sự để làm suy yếu bộ máy Mỹ - Ngụy. Vai trò của ông góp phần quan trọng tạo nên sức mạnh tổng hợp tiến tới giải phóng miền Nam."
    }
];
